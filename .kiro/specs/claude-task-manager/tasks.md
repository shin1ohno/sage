...
- [x] 13. TODOリスト管理システムの実装
- [x] 13.1 統合TODOリスト取得機能を実装
  - **テスト**: `tests/unit/todo-list-manager.test.ts`を先に作成
  - Apple RemindersとNotionからの統合タスク取得
  - プラットフォーム適応型のタスク一覧表示
  - _要件: 12.1, 12.2, 12.3_

- [x] 13.2 タスクフィルタリング機能を実装
  - **テスト**: フィルタリングのテストケースを先に作成
  - 優先度、期限、ステータス別のタスク絞り込み
  - 今日のタスク表示機能
  - _要件: 12.7, 12.8_

- [x] 13.3 タスクステータス更新機能を実装
  - **テスト**: ステータス更新のテストケースを先に作成
  - 統合ソースでのタスクステータス同期更新
  - 完了タスクの全ソース同期
  - _要件: 12.5, 12.6_

- [x] 13.4 list_todosツールを実装
  - **テスト**: list_todosのテストケースを先に作成
  - TODOリスト表示のメインツール
  - フィルタリングオプション付きタスク一覧
  - _要件: 12.1, 12.4_

- [x] 14. タスク同期システムの実装
- [x] 14.1 複数ソース間タスク同期機能を実装
  - **テスト**: `tests/unit/task-synchronizer.test.ts`を先に作成
  - Apple RemindersとNotion間のタスク同期
  - 競合検出と解決機能
  - _要件: 12.6_

- [x] 14.2 重複タスク検出機能を実装
  - **テスト**: 重複検出のテストケースを先に作成
  - 類似タスクの自動検出
  - 重複タスクのマージ提案
  - _要件: 12.1, 12.6_

- [x] 14.3 update_task_statusツールを実装
  - **テスト**: update_task_statusのテストケースを先に作成
  - タスクステータス更新のメインツール
  - 複数ソース同期機能
  - _要件: 12.5, 12.6_

- [x] 15. sync_to_notionツールの実装
- [x] 15. sync_to_notionツールの実装
- [x] 15.1 Notion MCP同期メインツールを実装
  - **テスト**: sync_to_notionのテストケースを先に作成
  - 長期タスクのNotion MCP経由同期機能
  - タスク情報の適切なMCPツール呼び出し
  - _要件: 8.1, 8.2, 8.3_

- [x] 15.2 MCP通信エラーハンドリングを実装
...
- [x] 17. エラーハンドリングとロバストネスの実装
- [x] 17. エラーハンドリングとロバストネスの実装
- [x] 17.1 統一エラーハンドリングシステムを実装
  - 全ツール共通のエラー処理機構
  - エラー分類と適切なメッセージ生成
  - _要件: 5.6, 8.5, 9.5_
...
- [x] 17.3 グレースフルデグラデーション機能を実装
  - 一部機能失敗時の他機能継続動作
  - 代替手段の提案機能
  - _要件: 7.3, 7.4, 7.5_

- [x] 18. マルチプラットフォーム対応の実装
- [x] 18. マルチプラットフォーム対応の実装
- [x] 18.1 Desktop/Code MCP版パッケージングを実装
  - MCPサーバー用のpackage.jsonとmanifest設定
  - 環境変数設定とエントリーポイント定義
  - _要件: 7.1, 7.4_

- [ ] 18.2 iOS/iPadOS Skills版パッケージングを実装
  - Claude Skills用のスクリプトとメタデータ
  - ネイティブ統合機能の適切な実装
  - _要件: 7.2, 7.4_

- [ ] 18.3 Web Skills版パッケージングを実装
  - 軽量版Skills実装とブラウザ互換性
  - 制限機能の適切な説明とフォールバック
  - _要件: 7.3, 7.4_

- [ ] 18.4 プラットフォーム間互換性テストを実装
  - 共通ロジックの全プラットフォーム動作確認
  - 設定データの互換性検証
  - _要件: 7.3, 7.4, 7.5_

- [x] 19. テストカバレッジ確認とE2Eテスト
- [x] 19. テストカバレッジ確認とE2Eテスト
...
- [x] 20. ドキュメントとデプロイメント準備
- [x] 20. ドキュメントとデプロイメント準備
- [x] 20.1 プラットフォーム別ユーザードキュメントを作成
