...
- [ ] 3. 設定管理システムの実装
- [ ] 3.1 プラットフォーム適応型設定管理を実装
  - **テスト**: `tests/unit/config-manager.test.ts`を先に作成
...
- [ ] 4. セットアップウィザードの実装
- [ ] 4.1 プラットフォーム適応型セットアップ状態確認を実装
  - **テスト**: `tests/unit/setup-wizard.test.ts`を先に作成
  - check_setup_statusツール（MCP）/セットアップ状態確認（Skills）
  - プラットフォーム別設定完全性チェック
  - _要件: 1.1, 1.6_

- [ ] 4.2 対話的セットアップウィザードを実装
...
- [ ] 9.1 プラットフォーム別Reminders統合基盤を実装
  - **テスト**: `tests/unit/reminders-platform-detection.test.ts`を先に作成
  - iOS/iPadOS: ネイティブ統合、macOS: AppleScript、Web: 手動コピー推奨
  - プラットフォーム検出と適切な統合方法の選択
  - _要件: 9.2, 9.3_

- [ ] 9.2 ネイティブReminders統合を実装（iOS/iPadOS Skills専用）
  - **テスト**: `tests/integration/native-reminders.test.ts`を先に作成（モック使用）
  - `window.claude?.reminders?.create()`を使用したネイティブ統合
  - 権限要求フローとエラーハンドリング
  - _要件: 9.2, 9.4, 9.5_

- [ ] 9.3 AppleScript Reminders統合を実装（Desktop MCP専用）
...
- [ ] 11.3 AppleScriptカレンダー統合を実装（Desktop MCP専用）
...
- [ ] 16. マルチプラットフォーム対応の実装
- [ ] 16.1 Desktop/Code MCP版パッケージングを実装
