...
| iOS/iPadOS Skills | 🔮 将来対応予定 | Claude Skills APIが利用可能になり次第 |
| Web Skills | 🔮 将来対応予定 | Claude Skills APIが利用可能になり次第 |

> **注意**: Claude Skills APIは現在サーバーサイドで実行され、iOSのネイティブフレームワーク（EventKit等）にはアクセスできません。iOS/iPadOS対応は、AnthropicがSkillsからデバイスAPIへのアクセスを提供した時点で実装予定です。
...
> **注意（将来対応）**: iOS/iPadOSでのネイティブCalendar統合は、Claude Skills APIがデバイスAPIへのアクセスを提供した時点で実装予定です。

### 要件7: クロスプラットフォーム互換性

**ユーザーストーリー:** Claude DesktopとClaude Codeの両方のユーザーとして、両方の環境で同じ機能を使いたい。どちらのツールを使っていても一貫したタスク管理ができるようにするため。

#### 受け入れ基準（現行実装: Desktop MCP）
...
> **プレースホルダー**: 以下の機能は、Claude Skills APIがデバイスAPIへのアクセスを提供した時点で実装予定です。

1. iOS/iPadOS Skills版が実装されたとき、システムはネイティブReminders/Calendar統合を使用すること
2. Web Skills版が実装されたとき、システムは手動コピー用テキスト生成を提供すること
3. プラットフォーム間で設定が同期されること（iCloud経由等）

### 要件8: Notion統合

**ユーザーストーリー:** Notionユーザーとして、長期タスクを自動的にNotionデータベースに同期してもらいたい。好みのツールでプロジェクトを管理できるようにするため。

#### 受け入れ基準（現行実装: Notion MCP Server経由）

1. タスクの期限が8日以上先のとき、システムはNotion MCP Server経由でNotionに同期すること
...
> **注意**: Notion統合にはNotion MCP Serverが事前に設定されている必要があります。iOS/iPadOSでのNotion Connector統合は将来対応予定です。

### 要件9: Apple Reminders統合

**ユーザーストーリー:** Appleエコシステムユーザーとして、短期タスクをApple Remindersに追加してもらいたい。すべてのデバイスでネイティブ通知を受け取れるようにするため。

#### 受け入れ基準（現行実装: macOS AppleScript）
...
> **注意（将来対応）**: iOS/iPadOSでのネイティブReminders統合は、Claude Skills APIがデバイスAPIへのアクセスを提供した時点で実装予定です。
